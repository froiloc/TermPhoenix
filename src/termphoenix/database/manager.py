...
class DatabaseManager:
    ...
    def _get_connection(self) -> sqlite3.Connection:
        ...
        if self.connection is None:
            raise DatabaseError("Database not initialized. Call initialize_project() first.")
        return self.connection
    
    def initialize_project(self, recreate_if_exists: bool = False) -> sqlite3.Connection:
        """
        Initialize a new project database or connect to existing one.
        
        Args:
            recreate_if_exists: If True, delete existing database and create new one
        ...
        try:
            ...
            if self.db_path.exists():
                if recreate_if_exists:
                    ...
                    logger.info(f"Connecting to existing database: {self.db_path}")
            ...
            logger.info(f"Project database initialized successfully: {self.db_path}")
            return self.connection
    ...
    def _configure_connection(self, conn: sqlite3.Connection):
        ...
        try:
            conn.create_function("regexp", 2, lambda pattern, string: 
                                1 if string and pattern and re.search(pattern, string) else 0)
        except:
            logger.warning("Regex support not available in SQLite")
    ...
    def create_crawl_session(self, root_url: str, parameters: Dict[str, Any]) -> int:
        """
    ...
    def get_or_create_website(self, domain: str, base_url: str = None) -> int:
        ...
        try:
            ...
            cursor.execute(
                "SELECT website_id FROM websites WHERE domain = ?",
                (domain,)
            )
            ...
            if result:
                ...
                # Update last_seen
                cursor.execute(
                    "UPDATE websites SET last_seen = datetime('now') WHERE website_id = ?",
                    (website_id,)
                )
                logger.debug(f"Found existing website: {domain} (ID: {website_id})")
            else:
                cursor.execute('''
                    INSERT INTO websites (domain, base_url, first_seen, last_seen)
                    VALUES (?, ?, datetime('now'), datetime('now'))
                ''', (domain, base_url))
                website_id = cursor.lastrowid
                logger.info(f"Created new website: {domain} (ID: {website_id})")
    ...
    def page_exists(self, url: str) -> bool:
        ...
        try:
            ...
            cursor = self.connection.cursor()
            cursor.execute(
                "SELECT page_id FROM pages WHERE url_hash = ?",
                (url_hash,)
            )
